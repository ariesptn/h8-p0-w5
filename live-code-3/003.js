...
function cart(shops) {
    ...
    for (let shop = 0; shop < shops.length; shop++) {
        if (listPurchase[shops[shop]] === undefined) {
            listPurchase[shops[shop]] = 1;
        } else {
            listPurchase[shops[shop]]++;
        }
    }
    for (let list in listPurchase) {
        let productIndex = 0
        for (productIndex = 0; productIndex < products.length; productIndex++) {
            if (products[productIndex].id === parseInt(list)) {
                break;
            }
        }
        result.carts.push({
            ...
            qty: listPurchase[list],
            subtotal: products[productIndex].price * listPurchase[list]
        })
        result.total += products[productIndex].price * listPurchase[list];
    }
    return result;
}
